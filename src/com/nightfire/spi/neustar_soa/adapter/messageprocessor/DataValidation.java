...
public class DataValidation extends DBMessageProcessorBase {
	...
	public void initialize ( String key, String type )throws ProcessingException
    {
		...
		strTemp = getRequiredPropertyValue( 
						SOAConstants.RANGE_TN_LOOPING_PROP , errorBuffer );
        ...
        if ( StringUtils.hasValue( strTemp ) )
        {
            ...
            try {
                ...
                errorBuffer.append ( "Property value for " 
					+ SOAConstants.RANGE_TN_LOOPING_PROP + " is invalid. " 
					+ e.getMessage ( ) + "\n" );
		...
		npaValue = getPropertyValue( SOAConstants.NPA_PROP );
        if( Debug.isLevelEnabled( Debug.SYSTEM_CONFIG ) ){
		  Debug.log( Debug.SYSTEM_CONFIG, "Value of NPA is ["	+ npaValue + "]." );
		}

		nxxValue = getPropertyValue( SOAConstants.NXX_PROP );
        
		if( Debug.isLevelEnabled( Debug.SYSTEM_CONFIG ) ){
	    	Debug.log( Debug.SYSTEM_CONFIG, "Value of NXX is ["	+ nxxValue + "]." );
		}

		startTNValue = getPropertyValue( SOAConstants.START_TN_PROP );
		...
		endTNValue = getPropertyValue( SOAConstants.END_TN_PROP );
		...
		validLoc = getRequiredPropertyValue( 
									SOAConstants.IS_VALID_PROP,errorBuffer );
        if( Debug.isLevelEnabled( Debug.SYSTEM_CONFIG ) ){
		...
		String positiveValProp = getRequiredPropertyValue( 
								SOAConstants.POSITIVE_VALIDATION_PROP , 
								errorBuffer );
                        						   ...
                        						   ProcessingException
    {
		...
		try
        {
			tmti = ThreadMonitor.start( "Message-processor [" + getName() + "] validating request" );
			...
			if ( useRangeLoop )
			{
				...
				for (int i = startTN; i <= endTN; i++ )
				{
					...
					if(mpContext.exists( errorLocation ))
					{
                        ...
                        if( Debug.isLevelEnabled(Debug.MSG_STATUS) ){
							Debug.log( Debug.SYSTEM_CONFIG,
												"Error Message from previous data validation component. ");
						}
						// Get the XML String
		...
		if( Debug.isLevelEnabled( Debug.SYSTEM_CONFIG ) ){
			Debug.log( Debug.SYSTEM_CONFIG, "Data Validation success : ");
		}
    										...
    										throws ProcessingException
    {
        ...
        try
		{
	        ...
	        for ( int Ix = 1;  iter.hasNext();  Ix ++ )
	        {
	            ...
	            }else if ( cd.columnType.equalsIgnoreCase(SOAConstants.COLUMN_TYPE_DATE) )
	            {
