/**
 * This is the Reply Handler, Once the SwimProcessingResultRecoveryReply for the N/W
 * data received by the NPAC COM server then it will initiate this reply handler.
 *  
 * @author Sreedhar K
 * @version 3.3
 ...
 * @seecom.nightfire.spi.neustar_soa.utils.TimeZoneUtil;
 */
...
3           Sreedhar         10/10/2005         Modified code to implement 
                                                swim_more_data.
...
public class NetworkDataSwimProcessingRecoveryReplyHandler extends
        NotificationHandler {
    ...
    */
    ...
    public NetworkDataSwimProcessingRecoveryReplyHandler 
                                              ( NPACAdapter adapter, 
                                                Session session, 
                                                List spids,
                                                int spidIndex,
     ...
     * method is called. It is  called before
     * the run method.
     * 
     * @param parsedNotification XMLMessageParser object received from 
     *                           notificaton.
     * @retun int
     */
    public int receiveNotification(XMLMessageParser parsedNotification) {
        ...
        if (ack == NPACConstants.ACK_RESPONSE) {
            ...
            try {
                ...
                (NPACConstants.SWIM_PROCESSING_RECOVERY_RESULTS_REPLY_STATUS);
                if(parsedNotification.exists(NPACConstants.
                        SWIM_PROCESSING_RECOVERY_RESULTS_REPLY_STOPTIME))
                {
                    String stopTime = notification.getTextValue
                   (NPACConstants.
                           SWIM_PROCESSING_RECOVERY_RESULTS_REPLY_STOPTIME);
                    
                    npacStopTime  = TimeZoneUtil.parse
                    (NPACConstants.UTC,NPACConstants.UTC_TIME_FORMAT,stopTime);
                }
     ...
     * to our SwimRequest for net work data. This simply retries the Swim 
     * request for net work data.
     *
    ...
    protected void handleError(XMLMessageParser error) {
        ...
        adapter.retryNetWorkDataSwimRequest(session,
                                            region, 
                                            spids,
                                            spidIndex,
    ...
    public void timeout() {
        ...
        adapter.sendNetworkDataSwimRequest(session,
                                           region,
                                           spids, 
                                           spidIndex,
                                           recoveryCompleteTime, 
     ...
     *  for the swim  request was successful. If not successful, 
     *  then this retries the original Swim request for net work data.
     */
