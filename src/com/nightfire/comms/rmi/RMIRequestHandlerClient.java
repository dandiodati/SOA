...
/**
 * Makes a synchronous or asynchronous RMI call with a text payload.
 */
public class RMIRequestHandlerClient extends MessageProcessorBase
{
    /**
        ...
        throws MessageException, ProcessingException
    {
        ...
        if ( data == null )
        {
            throw new MessageException( "No input data available to send via RMI remote call." );
        }
        ...
        try
        {
            try
            {
                result = invokeServer( header, data );
            }
            catch ( Exception e  )
            {
     ...
     */
    private String invokeServer ( String header, String request ) throws Exception
    {
        if ( Debug.isLevelEnabled( Debug.IO_DATA ) )
            Debug.log( Debug.IO_DATA, "Header value is:\n" + header + 
                       "\nRequest is string of length [" + request.length() + "]." );
        ...
        else
        {
            ...
            String response = getServer().processSync( header, request );

            if ( response != null )
            {
                if ( Debug.isLevelEnabled( Debug.IO_DATA ) )
                        Debug.log( Debug.IO_DATA, "Response is String of length [" + response.length() + "]." );
            }
     ...
     *
     * @return  A reference to a RMIRequestHandler object.
     *
     * @exception Exception Thrown if processing fails
     */
    private RMIRequestHandler getServer ( ) throws Exception
    {
        String serviceName = null;

        if ( server == null )
        {
            synchronized ( RMIRequestHandlerClient.class )
            {
                if ( server == null )
                {
                    ...
                    server = (RMIRequestHandler)Naming.lookup( serviceName );
                }
            }

            if ( server == null )
            {
                throw new ProcessingException( "Lookup of RMI server [" + serviceName + "] failed." );
            }
    ...
    private static RMIRequestHandler server = null;
} 
