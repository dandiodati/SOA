...
/**
 *  This tag checks the header field "ApplyTemplates" of bean bag. 
 *  Each order's  body data source generator gets checked in here. 
 *  If "template application" was toggled, XML generator sets accordingly.
 */

public class PrepareTemplateDataTag extends VariableTagBase implements ManagerServletConstants, ServletConstants
{        
    ...
    public int doStartTag() throws JspException
    {
        ...
        String headerApplyTemplates = existingBean.getHeaderValue(ManagerServletConstants.APPLY_TEMPLATES_HEADER_FIELD);

        if(StringUtils.hasValue(headerApplyTemplates))
        {
            if("true".equalsIgnoreCase(headerApplyTemplates))
            {
                applyTemplates = true;
            }
        }
            ...
            for(int i =0; i < compDefsList.size();++i)
            {
                ...
                if (serviceComponentBeans != null && serviceComponentBeans.size() > 0)
                {
                    ...
                    while(iter.hasNext() )
                    {
                        ...
                        if( false == applyTemplates && (bean.getBodyDataSource() instanceof XMLTemplateGenerator)) 
                        {
                            try{
                            bean.setBodyDataSource(new XMLPlainGenerator(((XMLTemplateGenerator)bean.getBodyDataSource()).getOutput()));
                            }
                            catch(Exception e)
                            {
                        ...
                        else if (true == applyTemplates && !(bean.getBodyDataSource() instanceof XMLTemplateGenerator)) 
                        {
                            ...
                            String keyPath = ((new StringBuffer(resourceRoot)).append('/').append(cmpDef.getID()).append('-').append(bean.getHeaderValue(SUPPLIER))).toString();
