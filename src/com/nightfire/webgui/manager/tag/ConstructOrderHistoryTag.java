...
/**
 * ConstructOrderHistoryTag is responsible for outputing the history of a service
 * in an HTML table format.
 */

public class ConstructOrderHistoryTag extends ConstructServiceInfoTag implements PlatformConstants
{
    ...
    public int doStartTag() throws JspException
    {
        ...
        if (StringUtils.hasValue(historyDisplayStatus) && historyDisplayStatus.equals(ServiceComponentBean.EXPAND_STATUS))
        {
            ...
            if (serviceHistory != null)
            {
                ...
                {
                    log.error("ConstructOrderHistoryTag.doStartTag(): Failed to parse the obtained service history for display:\n" + e.getMessage());
                }
            }
            ...
            {
                String errorMessage = "ERROR: ConstructOrderHistoryTag.doStartTag(): Failed to write to JspWriter output-stream.\n" + e.getMessage();
	 ...
	 protected String getDetailFieldValue(XMLGenerator serHist, Node n, Field field) throws JspTagException
	{
		try{
			...
			return serHist.getValue(n, fieldName);
                ...
                String errorMessage = "ERROR: ConstructOrderHistoryTag.getDetailFieldValue(): Failed to get value of detail field [" + field.getXMLPath() + "] .\n" + e.getMessage();
	...
	*/
	protected Object[] getDetailFields() {
		...
		return serviceDef.getHistoryInfo().getSummaryFields().toArray();
	}
    ...
    private XMLGenerator getServiceHistory()
    {
        try
        {
            ...
            if (serviceHistory == null)
            {
				...
				if (log.isDebugEnabled())
				{
					log.debug("ConstructOrderHistory.getServiceHistory(): The processing layer returned the following response:\n");

					log.debug("ConstructOrderHistory.getServiceHistory(): Header:\n" + response.getHeaderStr());

					log.debug("ConstructOrderHistory.getServiceHistory(): Body:\n" + response.getBodyStr());
				}
        ...
        {
            log.error("ConstructOrderHistory.getServiceHistory(): Failed to obtain the service history:\n" + e.getMessage());
        }
