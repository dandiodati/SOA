 ...
 *
 */
public class LoadBundleBeanDetailTag extends VariableTagBase implements ServletConstants, ManagerServletConstants
{
    ...
    */
    ...
    public int doStartTag() throws JspException
    {
      super.doStartTag();
	   ...
	   if ( bundleData != null) {
          ...
          String err = "LoadBundleBeanDetailTag : Failed to create Bundle bean bag data: " + e.getMessage();
          log.error(err);
          log.error("",e);
    ...
    public void setupBundleInfo(BundleBeanBag bag) throws JspException, ServletException
    {
      ...
      if ( !loadedBundleDef ) {
            String errorMessage = "LoadBundleBeanDetailTag: No BundleDef object with name [" + bundleMDN + "] exists in the servlet-context's [" + BUNDLE_DEFS + "] lookup.";
     ...
     {   
      ...
      while (bIter.hasNext() ) {
	   ...
	   if (!StringUtils.hasValue(messageType) ) {
         ...
         String keyPath = resourceRoot +'/' +serviceType +"-" + supplier;

         try {
            setMetaData(bean, keyPath, messageType);
            setTransformers(bean, keyPath, messageType);

            if ( loadTemplates )
               setTemplateData(bean, keyPath, messageType);
     ...
     */
    protected void setMetaData(ServiceComponentBean bean, String keyPath, String messageType) throws Exception
    {
     ...
     * @exception  Exception  Thrown when an error occurs during processing.
     */
    protected void setTransformers(ServiceComponentBean bean, String keyPath, String messageType) throws Exception
    {


        String incomingBodyTransformPath = ((new StringBuffer(keyPath)).append('/').append(XSL_DIRECTORY).append('/').append(messageType).append(INCOMING_XSL_SUFFIX).append(".xsl")).toString();

        String outgoingBodyTransformPath = ((new StringBuffer(keyPath)).append('/').append(XSL_DIRECTORY).append('/').append(messageType).append(OUTGOING_XSL_SUFFIX).append(".xsl")).toString();
        String outgoingHeaderTransformPath = ((new StringBuffer(keyPath)).append('/').append(XSL_DIRECTORY).append('/').append(HEADER_TRANSFORMER_XSL)).toString();
     ...
     * @exception  Exception  Thrown when an error occurs during processing.
     */
    protected void setTemplateData(ServiceComponentBean bean, String keyPath, String messageType) throws Exception
    {
